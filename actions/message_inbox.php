...
else
{
	...
	{	
		...
		echo '<tr>
			...
			<th id="delete">Delete Message</th>
			</tr>';
		// Show the users messages
		while ($message=mysql_fetch_assoc($result))
		{
			echo '<tr>';
			echo '<td class="action"><a href="/?action=user_profile&amp;user_id='.$message['user_id_from'].'" class="edit">'.$message['username'].'</a></td>';
			...
			echo '<td class="action"><a href="/?action=message_delete&amp;message_id='.$message['message_id'].'" class="delete">Delete</a></td>';
			echo '</tr>';
		}
