...
class Ajax_IM_Admin {
   ...
   function Ajax_IM_Admin($call) {
      if($_SESSION['level'] <= 4) print 'access_denied';
   ...
   function search($by, $for) {
      ...
      $search_query = mysql_query('SELECT username, email, last_ip, last_ping, is_online, admin, banned FROM users WHERE ' . $by . ' LIKE \'' . mysql_real_escape_string(str_replace('*', '%', $for)) . '\'');
      ...
      while($row = mysql_fetch_assoc($search_query)) {
         $found[] = array('username'=>$row['username'], 'email'=>$row['email'], 'lastKnownIP'=>$row['last_ip'], 'lastActive'=>$row['last_ping'],
                          'currentStatus'=>$row['is_online'], 'banned'=>($row['level']==0?'true':'false'), 'admin'=>($row['level']>=4?'true':'false'));
      }
