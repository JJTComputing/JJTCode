...
/* ajax im - modified for JJTCode by Tristan Millington */

// Load config files - and JJTCode functions
require('config.php');
...
class Ajax_IM {
   ...
   function logout() {
      ...
      $set_status = mysql_query('UPDATE users SET is_online=0, last_ping=\'' . time() . '\' WHERE username=\'' . mysql_real_escape_string($this->username) . '\'');
   ...
   function ping($away) {
      ...
      $set_status = mysql_query('UPDATE users SET is_online=\'' . mysql_real_escape_string($away + 1) . '\', last_ping=\'' . time() . '\' WHERE username=\'' . mysql_real_escape_string($this->username) . '\'');
   ...
   function send($to, $message, $font, $font_size, $font_color, $bold, $italic, $underline, $is_chat) {
      ...
      if($is_online > 0 || $is_chat == 'true') {
         if($is_online == 100) {
            $check_friendship = mysql_query('SELECT is_online FROM users WHERE username IN(SELECT user FROM ' . SQL_PREFIX . 'buddylists WHERE user=\'' . strtolower($to) . '\' AND buddy=\'' . mysql_real_escape_string($this->username) . '\')');
   ...
   function isUser($username) {
      ...
      $query = mysql_query('SELECT is_online FROM users WHERE username=\'' . mysql_real_escape_string($username) . '\'');
      
      if(mysql_num_rows($query) > 0) {
         $userinfo = mysql_fetch_assoc($query);
         
         if($userinfo['is_online'] == 100) {
            $check_friendship = mysql_query('SELECT is_online FROM users WHERE username IN(SELECT user FROM ' . SQL_PREFIX . 'buddylists WHERE user=\'' . mysql_real_escape_string($username) . '\' AND buddy=\'' . mysql_real_escape_string($this->username) . '\')');
   ...
   function reset($email) {
      ...
      $query = mysql_query('SELECT email FROM users WHERE email=\'' . mysql_real_escape_string($email) . '\'');
      if(mysql_num_rows($query) > 0) {
         $new_pass = $this->generatePassword();
         $query = mysql_query('UPDATE users SET password=\'' . md5($new_pass) . '\' WHERE email=\'' . mysql_real_escape_string($email) . '\'');
   ...
   function passwordChange($password, $new_password) {
      ...
      if(strlen($new_password) >= 6 && strlen($new_password) <= 20) {
         $query = mysql_query('UPDATE users SET password=\'' . md5($new_password) . '\' WHERE username=\'' . mysql_real_escape_string($this->username) . '\'');
         return 'pw_changed';
      } else {
   ...
   function changeIcon() {
      ...
      */
      ...
      if (move_uploaded_file($_FILES['icon']['tmp_name'], "./buddyicons/".$this->username.".".$extension)) {
         if (mysql_query('UPDATE users SET buddyicon=\'' . mysql_real_escape_string($extension) . '\' WHERE username=\'' . mysql_real_escape_string($this->username) . '\'')) {
            return "success";
         } else {
            return "unkown";
         }
   ...
   function changeProfile($profile) {
      if (mysql_query('UPDATE users SET profile=\'' . mysql_real_escape_string(strip_tags($profile)) . '\' WHERE username=\'' . mysql_real_escape_string($this->username) . '\'')) {
         return 'success';
      } else {
         return 'failed';
      }
   ...
   function getProfile($username) {
      $query = mysql_query('SELECT profile FROM users WHERE username=\'' . mysql_real_escape_string($username) . '\'');
   ...
   function userEvent($username, $buddylist, $event, $args) {      
      ...
      switch($event) {
         case 'chat':
            ...
            $query = mysql_query('SELECT username, is_online FROM users WHERE username IN(\'' . $users_str . '\') AND is_online > 0 ORDER BY username ASC');
         ...
         case 'status':
            ...
            foreach($buddylist as $group => $users) {
               ...
               $query = mysql_query('SELECT username, is_online FROM users WHERE (' . $query_string . $users_str . '\')) GROUP BY username');
   ...
   function gc() {
      ...
      if(rand(1, 100) <= 30) {
         ...
         $cleanup_event = mysql_query('SELECT username FROM users WHERE last_ping < ' . $expire_time . ' AND is_online > 0');
         ...
         $cleanup = mysql_query('UPDATE users SET is_online=0 WHERE last_ping < ' . $expire_time . ' AND is_online > 0');
      }
   }
}
